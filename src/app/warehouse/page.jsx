...
export default function WarehousePartnerPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/proxy/api/warehouse/job-application', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <section className="relative bg-gradient-to-br from-brand-500 via-brand-600 to-indigo-700 py-24 overflow-hidden">
        {/* Background Elements */}
        <div className="absolute inset-0 bg-black/10"></div>
        <div className="absolute top-0 left-0 w-72 h-72 bg-brand-400/20 rounded-full -translate-x-1/2 -translate-y-1/2"></div>
            ...
            {!showRegistration ? (
              <div className="space-y-6">
                <button
                  ...
                  className="group bg-white text-brand-600 px-14 py-5 rounded-2xl font-bold text-lg hover:bg-gray-50 transition-all duration-300 shadow-2xl hover:shadow-3xl hover:scale-105 transform"
                >
              ...
              <div className="bg-white rounded-3xl shadow-2xl border border-gray-200 p-8 max-w-2xl mx-auto transform hover:scale-[1.02] transition-transform duration-300">
                ...
                <form onSubmit={handleSubmit}>
                  <div className="grid gap-6 mb-8">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-4 py-3.5 border-2 border-gray-200 rounded-xl focus:border-brand-500 focus:outline-none focus:ring-4 focus:ring-brand-100 transition-all text-gray-800 placeholder-gray-500"
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={handleChange}
                          className="w-full px-4 py-3.5 border-2 border-gray-200 rounded-xl focus:border-brand-500 focus:outline-none focus:ring-4 focus:ring-brand-100 transition-all text-gray-800"
                          required
                        >
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full bg-gradient-to-r from-brand-600 to-brand-700 text-white font-bold py-4 rounded-xl hover:from-brand-700 hover:to-brand-800 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed text-lg shadow-lg hover:shadow-xl transform hover:scale-[1.02]"
                  >
                    {loading ? (
                      <span className="flex items-center justify-center">
                        <LoadingDots size="sm" className="text-white mr-3" />
