...
const ContactModal = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/proxy/api/contact/submit', {
        method: 'POST',
  ...
  const inputClass = (field) =>
    `block w-full px-4 py-2.5 rounded-lg border text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-opacity-50 transition-colors ${
      formErrors[field]
        ...
        : 'border-gray-300 focus:border-brand-500 focus:ring-brand-400'
    }`;
        ...
        <div className="px-6 py-5">
          ...
          {status.message && (
            <div className={`mb-5 p-4 rounded-xl flex items-start gap-3 ${
              ...
              {status.type === 'success'
                ? <CheckCircleIcon className="w-5 h-5 text-brand-500 shrink-0 mt-0.5" />
                : <ExclamationCircleIcon className="w-5 h-5 text-red-500 shrink-0 mt-0.5" />
              }
              <div>
                <p className={`text-sm font-medium ${status.type === 'success' ? 'text-brand-800' : 'text-red-800'}`}>
                  {status.message}
                </p>
                {status.referenceId && (
          ...
          {status.type !== 'success' && (
            <form onSubmit={handleSubmit} className="space-y-4" noValidate>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3 px-4 rounded-xl font-semibold text-white bg-brand-600 hover:bg-brand-700 disabled:opacity-60 disabled:cursor-not-allowed transition-colors shadow-sm shadow-brand-200 flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
          ...
          {status.type === 'success' && (
            <button
              onClick={handleClose}
              className="mt-2 w-full py-3 px-4 rounded-xl font-semibold text-brand-600 border border-brand-200 hover:bg-brand-50 transition-colors"
            >
              Close
            </button>
          )}
