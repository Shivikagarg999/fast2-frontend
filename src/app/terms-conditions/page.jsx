...
export default function TermsAndConditionsPage() {
  ...
  const fetchAllPolicies = async () => {
    try {
      setLoading(true);
      const response = await fetch('https://www.fast2.in/proxy/api/policy/active', {
        method: 'GET',
        headers: {
  ...
  const getPolicyTitle = (policyType) => {
    switch(policyType) {
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="py-12 bg-gradient-to-b from-green-50 to-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <nav className="flex overflow-x-auto scrollbar-hide">
                  {tabs.map((tab) => (
                    <button
                      ...
                      className={`flex-1 min-w-[200px] py-6 px-4 text-center font-medium text-lg transition-all duration-300 ${
                        activeTab === tab.id
                          ? `bg-gradient-to-r ${getPolicyColor(tab.id)} text-white`
                ...
                {!currentPolicy ? (
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-xl font-semibold text-gray-900 mb-2">
                      No {getPolicyTitle(activeTab)} Available
                    </h3>
                  ...
                  <>
                    {/* Policy Header */}
                    <div className={`bg-gradient-to-r ${getPolicyColor(activeTab)} text-white rounded-2xl p-8 mb-8`}>
                      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                          <div className="flex items-center gap-3 mb-4">
                            <span className="text-3xl">{getPolicyIcon(activeTab)}</span>
                            <h2 className="text-2xl font-bold">{currentPolicy.title}</h2>
                          </div>
                        ...
                        <div className="text-right">
                          ...
                          <p className="font-semibold">{formatDate(currentPolicy.effectiveDate)}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    {currentPolicy.metadata && (
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                        ...
                        {currentPolicy.metadata.cancellationFee && (
                          <div className="bg-red-50 border border-red-100 rounded-xl p-4">
                            <p className="text-sm text-red-600 font-medium mb-1">Cancellation Fee</p>
                            <p className="text-lg font-semibold text-gray-900">{currentPolicy.metadata.cancellationFee}%</p>
                          </div>
                        )}
                        {currentPolicy.metadata.refundProcessingDays && (
                          <div className="bg-amber-50 border border-amber-100 rounded-xl p-4">
                            <p className="text-sm text-amber-600 font-medium mb-1">Refund Processing</p>
                            <p className="text-lg font-semibold text-gray-900">{currentPolicy.metadata.refundProcessingDays} business days</p>
                          </div>
                        )}
                    ...
                    <div className="prose prose-lg max-w-none">
                      ...
                      <div className="bg-gray-50 rounded-xl p-4 mb-8 border border-gray-200">
                        <p className="text-gray-700 text-sm">
                          <span className="font-semibold">Last Updated:</span> {formatDate(currentPolicy.updatedAt || currentPolicy.createdAt)}
                        </p>
                      </div>
                    ...
                    <div className="mt-12 pt-8 border-t border-gray-200">
                      <div className={`bg-gradient-to-r ${getPolicyColor(activeTab)}/10 rounded-2xl p-6 border ${getPolicyColor(activeTab).replace('from-', 'border-').replace(' to-', '/20')}`}>
                        <h3 className="text-lg font-semibold text-gray-900 mb-4">
                          {activeTab === 'terms' ? 'Acceptance of Terms' : 'Important Notice'}
                        </h3>
                      ...
                      <div className="grid md:grid-cols-2 gap-4">
                        <div className="bg-white rounded-xl p-4 border border-gray-200">
                          ...
                          <p className="font-medium text-gray-900">
                            {currentPolicy.metadata?.contactEmail || 'support@fast2.in'}
                          </p>
                        </div>
                        <div className="bg-white rounded-xl p-4 border border-gray-200">
          ...
          {!loading && !error && (
            <div className="mb-12">
              <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">All Active Policies</h2>
                ...
                {tabs.map((tab) => {
                  ...
                  return (
                    <div 
                      key={tab.id}
                      onClick={() => setActiveTab(tab.id)}
                      className={`bg-white rounded-2xl p-6 border-2 cursor-pointer transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ${
                        activeTab === tab.id 
                          ? `border-${getPolicyColor(tab.id).split('-')[1]} shadow-lg` 
