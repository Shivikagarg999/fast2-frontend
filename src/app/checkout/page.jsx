...
const CheckoutPage = () => {
  ...
  const getAddressTypeColor = (type) => {
    switch (type) {
      case 'home': return 'text-purple-600 bg-purple-50 border-purple-200';
      case 'work': return 'text-purple-600 bg-purple-50 border-purple-200';
          ...
          <div className="border-b border-gray-200 bg-white">
            <div className="flex justify-center items-center p-6">
              <div className={`flex items-center ${step >= 1 ? 'text-purple-600' : 'text-gray-400'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${step >= 1 ? 'border-purple-600' : 'border-gray-300'}`}>
              ...
              <div className="flex-1 h-1 bg-gray-200 mx-4 max-w-20">
                <div className={`h-1 ${step >= 2 ? 'bg-purple-600' : 'bg-gray-200'}`}></div>
              </div>

              <div className={`flex items-center ${step >= 2 ? 'text-purple-600' : 'text-gray-400'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${step >= 2 ? 'border-purple-600' : 'border-gray-300'}`}>
                  2
                </div>
                ...
                <div>
                            ...
                            >
                              <div className="flex items-start justify-between">
                                ...
                                {selectedAddress?._id === address._id && (
                                  <CheckCircleSolidIcon className="w-5 h-5 text-purple-600 flex-shrink-0 ml-2" />
                                )}
                              </div>
                            </div>
                          );
                  ...
                  {!showAddressForm && savedAddresses.length === 0 && (
                    <div className="mb-8">
                      <div
                        onClick={handleAddNewAddress}
                        className="border-2 border-dashed border-purple-300 rounded-xl p-8 text-center cursor-pointer hover:border-purple-400 transition-colors bg-purple-50"
                      >
                        <PlusIcon className="w-8 h-8 text-purple-600 mx-auto mb-2" />
                  ...
                  {isMedicalOrder() && (
                    <div className="bg-purple-50 border border-purple-200 rounded-xl p-6 mb-6">
                      <div className="flex items-center mb-4">
                        <CheckBadgeIcon className="w-6 h-6 text-purple-600 mr-2" />
                        <h3 className="text-lg font-bold text-purple-900">Medical Order - Prescription Required</h3>
                      </div>
                      ...
                      <div className="flex items-start space-x-4">
                        <div className="w-32 h-32 border-2 border-dashed border-purple-300 rounded-xl flex items-center justify-center bg-white relative overflow-hidden flex-shrink-0">
                          {prescriptionPreview ? (
                            <img src={prescriptionPreview} alt="Prescription preview" className="w-full h-full object-cover" />
                          ) : (
                            <PencilIcon className="h-8 w-8 text-purple-300" />
                          )}
                        </div>
                        <div className="flex-1">
                          <input
                          ...
                          <label
                            ...
                            className="inline-flex items-center px-6 py-3 bg-purple-600 text-white rounded-lg font-semibold hover:bg-purple-700 cursor-pointer shadow-sm transition-all"
                          >
                            <PlusIcon className="w-5 h-5 mr-2" />
                            {prescriptionImage ? 'Change Prescription' : 'Upload Prescription'}
                          </label>
                          <p className="text-xs text-purple-500 mt-2">Maximum file size: 5MB (JPG, PNG)</p>
                          {prescriptionImage && (
                            <button
                      ...
                      <div className="bg-purple-50 border border-purple-200 rounded-xl p-6">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center">
                            ...
                            <div className="flex items-center">
                              {useWallet ? <WalletSolidIcon className="w-5 h-5 text-purple-600 mr-2" /> : <WalletIcon className="w-5 h-5 text-purple-600 mr-2" />}
                              <span className="font-semibold text-purple-800">Use Wallet Balance</span>
                            </div>
                          </div>
                          <span className="font-semibold text-purple-700">₹{walletBalance}</span>
                        </div>
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center">
                            ...
                            <div className="flex items-center">
                              <BanknotesIcon className={`w-5 h-5 ${paymentMethod === 'cod' ? 'text-purple-600' : 'text-gray-600'} mr-2`} />
                              <span className={`font-semibold ${paymentMethod === 'cod' ? 'text-purple-800' : 'text-gray-800'}`}>
                                Cash on Delivery
                              </span>
                            </div>
                          </div>
                          <span className={`font-medium ${paymentMethod === 'cod' ? 'text-purple-700' : 'text-gray-700'}`}>
                            {useWallet && walletBalance > 0 ? `₹${amountBreakdown.payableAmount} to pay` : 'Pay with cash'}
                          </span>
                        </div>
                        <p className={`text-sm mt-3 ml-9 ${paymentMethod === 'cod' ? 'text-purple-600' : 'text-gray-600'}`}>
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center">
                            ...
                            <div className="flex items-center">
                              <CreditCardIcon className={`w-5 h-5 ${paymentMethod === 'online' ? 'text-purple-600' : 'text-gray-600'} mr-2`} />
                              <span className={`font-semibold ${paymentMethod === 'online' ? 'text-purple-800' : 'text-gray-800'}`}>
                                Pay Online
                              </span>
                            </div>
                          </div>
                          <div className="flex items-center space-x-2">
                            ...
                            <span className={`font-medium ${paymentMethod === 'online' ? 'text-purple-700' : 'text-gray-700'}`}>
                              Secure Payment
                            </span>
                          </div>
                        </div>
                        <p className={`text-sm mt-3 ml-9 ${paymentMethod === 'online' ? 'text-purple-600' : 'text-gray-600'}`}>
                  ...
                  <button
                    ...
                    className="w-full bg-purple-600 text-white py-4 px-6 rounded-xl hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 font-semibold disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm"
                  >
                    {processing ? (
                ...
                <div className="space-y-3 border-t border-gray-200 pt-4">
                  ...
                  <div className="flex justify-between">
                    ...
                    <div className="text-right">
                      {amountBreakdown.subtotal > 199 ? (
                        <div>
                          <span className="font-medium text-purple-600 line-through text-sm">₹{amountBreakdown.deliveryCharges}</span>
                          <span className="font-medium text-purple-600 ml-2">FREE</span>
                        </div>
                      ) : (
                  ...
                  <div className="border border-dashed border-gray-300 rounded-lg p-3">
                    {appliedCoupon ? (
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-xs font-semibold text-purple-700">🎟️ Scratch Coupon Applied</p>
                      ...
                      <div>
                        <div className="flex gap-2">
                          <input
                            ...
                            className="flex-1 text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-300 uppercase placeholder-normal"
                          />
                          <button
                            onClick={handleApplyCoupon}
                            disabled={couponLoading || !couponCode.trim()}
                            className="bg-purple-600 text-white text-sm font-semibold px-4 py-2 rounded-lg hover:bg-purple-700 transition-colors disabled:opacity-50 whitespace-nowrap"
                          >
                            {couponLoading ? '...' : 'Apply'}
                          </button>
                        </div>
                  ...
                  <div className="flex justify-between pt-3 border-t border-gray-200">
                    ...
                    <span className="text-lg font-bold text-purple-600">₹{amountBreakdown.payableAmount}</span>
                  </div>
    ...
    {step === 2 && (
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-3xl shadow-2xl max-w-sm w-full p-8 text-center overflow-hidden">
          ...
          <div className="absolute -top-10 -right-10 w-40 h-40 bg-purple-100 rounded-full opacity-60"></div>
          ...
          <div className="absolute top-5 left-10 w-2 h-2 bg-purple-400 rounded-full"></div>
          ...
          <div className="relative w-24 h-24 mx-auto mb-5">
            <div className="w-24 h-24 bg-purple-100 rounded-full flex items-center justify-center">
              <div className="w-16 h-16 bg-purple-500 rounded-full flex items-center justify-center shadow-lg shadow-purple-300">
          ...
          {/* Payment Info */}
          <div className="relative bg-purple-50 border border-purple-100 rounded-2xl px-5 py-3 mb-6">
            <p className="text-[10px] text-purple-600 uppercase tracking-widest font-semibold mb-0.5">
          ...
          <div className="relative flex gap-3">
            ...
            <button
              onClick={() => router.push('/')}
              className="flex-1 bg-purple-500 hover:bg-purple-600 text-white py-3 px-4 rounded-xl font-semibold text-sm transition-colors shadow-lg shadow-purple-200"
            >
              Shop More
            </button>
          </div>
