...
const MyCouponsPage = () => {
  ...
  useEffect(() => {
    const fetchCoupons = async () => {
      ...
      try {
        const res = await fetch('/proxy/api/order/my-scratch-coupons', {
      ...
      <div className="min-h-screen bg-[#f4f7f4] flex items-center justify-center px-4">
        <div className="text-center max-w-sm">
          ...
          <button
            ...
            className="bg-brand-600 text-white px-6 py-2.5 rounded-xl font-semibold text-sm hover:bg-brand-700 transition-colors"
          >
            Try Again
          </button>
        </div>
    ...
    <div className="min-h-screen bg-[#f4f7f4] pb-20">
      ...
      <div className="bg-gradient-to-r from-[#1a3d1a] to-[#2e6b2e]">
        <div className="max-w-2xl mx-auto px-4 py-7">
          <button
            onClick={() => router.back()}
            className="flex items-center text-brand-300 text-sm mb-4 hover:text-white transition-colors"
          >
            <ArrowLeftIcon className="w-4 h-4 mr-1" />
            Back
          </button>
          <p className="text-brand-300 text-xs font-semibold uppercase tracking-widest mb-1">Rewards</p>
          <h1 className="text-2xl font-bold text-white">My Scratch Coupons</h1>
          <p className="text-brand-200/70 mt-0.5 text-sm">
            {coupons.length === 0
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        ...
        {coupons.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-10 text-center">
            ...
            <button
              onClick={() => router.push('/')}
              className="bg-brand-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-brand-700 transition-colors inline-flex items-center text-sm shadow-sm shadow-brand-200"
            >
...
const CouponCard = ({ coupon, onCopy, copied, onUseNow }) => {
  const statusConfig = {
    scratched: { label: 'Available', cls: 'bg-brand-100 text-brand-700' },
